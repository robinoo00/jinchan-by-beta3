...
export default {
    ...
    effects: {
        ...
        *validate({},{put,call,select}){
            ...
            //校验地区
            // const c5 = yield select(state => state.register.province_choose);
            // if(!c5){
            //     Toast.info('请选择区域',1)
            //     return;
            // }
        ...
        * submit({values}, {put, call, select}) {
            ...
            //城市赋值
            // const c5 = yield select(state => state.register.province_choose);
            // const dq2 = yield select(state => state.register.city_choose);
            // const dq3 = yield select(state => state.register.district_choose);
            // formData.append('c5',c5)
            // formData.append('dq2',dq2)
            // formData.append(dq3,dq3)
            //赋值图片
            const tp_list = yield select(state => state.register.tp_list);
            for (let key in tp_list) {
                let item = tp_list[key];
